...
export class ShowHandler {
  ...
  static searchedProducts(texto, products) {
    ...
    products.forEach((product) => {
      if (product.nome.toLowerCase().includes(texto.toLowerCase())) {
        ...
        product.descricao.toLowerCase().includes(texto.toLowerCase())
      ) {
        searchedProducts.push(product);
      }
    });
  ...
  static headerMain(verify) {
    ...
    rightSide__icon.innerHTML = `<img src="${
      this.page === "Home"
    ...
    if (this.page === "Home") {
      ...
      const cartOpen__quantity = document.createElement("span");
      const cartOpen__icon = document.createElement("img");

      cartOpen.classList.add("cartOpen");
      cartOpen__quantity.classList.add("cartOpen__quantity");
      cartOpen__quantity.innerText = "0";
      cartOpen__icon.classList.add("cartOpen__icon");
      cartOpen__icon.src = `${
        this.page === "Home"
          ? "./src/assets/images/cartOpen.png"
          : "../assets/images/cartOpen.png"
      }`;
      cartOpen__icon.alt = "open cart icon";

      rightSide.append(rightSide__search, cartOpen);
      cartOpen.append(cartOpen__quantity, cartOpen__icon);
    } else {
  ...
  static async showProducts(products, verify) {
    ...
    products.forEach((product) => {
      ...
      card__btn.innerHTML =
        '<img src="src/assets/images/littleCart.png" alt="add cart button">';
  ...
  static async getProducts(verify) {
    ...
    if (verify) {
      list = await ProductService.getPrivateProducts(
  ...
  static changeTheSelected(category) {
    document
      ...
      .forEach((item) => item.classList.remove("menu__item--selected"));
